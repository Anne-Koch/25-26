...
- In KW 20 (ab 11.05.2026) gibt es die Kick-off Tutorien zum Kennenlernen ihrer Gruppen
- Freitag den 08.05.2026 wird das 4. Projekt (des Gesamtkurses) für jede Gruppe auf Github hochgeladen
- Freitag den 08.05.2026 wird das 4. Projekt besprochen (Vorstellung und Interpretationen)
- Bearbeitungszeit des 4. Projekts: Bis 14.05.2026
- Freitag 15.05.2026 wird das 5. Projekt vorgestellt (Online)
- Mittwoch den 20.05.2026 gibt es die Probeklausur im Hörsaal ab 08:30 Uhr. Teil der Vorleistung, mind. 20% der Punkte müssen bestanden werden, keine Hilfmittel! 
- Ab KW 21 finden die Tutorien statt (ab dem 5. Projekt)
- Bearbeitungszeit für das 5. Projekt: Bis 04.06.2026
...
add_rows_df <- data.frame(
  term = "Kontrollvariablen",
  OLS  = "Ja", `2SLS händisch` = "Ja", `2SLS automatisch` = "Ja",
  check.names = FALSE
)

modelsummary(
  list(ols_baseline, second_stage, iv_2sls),  # unnamed → generates (1)(2)(3) as col IDs
  output     = "gt",
  ...
  coef_rename = c("educ" = "Bildung", "bildung_hat" = "Bildung (IV)"),
  add_rows   = add_rows_df,
  title      = "Log(Einkommen) auf Bildung regressiert"
) |>
  cols_label(
    `(1)` = "OLS",
    `(2)` = md("2SLS  \nhändisch"),       # md() renders markdown line breaks
    `(3)` = md("2SLS  \nautomatisch")
  )
```
]
